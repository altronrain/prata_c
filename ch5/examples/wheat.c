...
#include <stdio.h>
#define SQUARES 64				/* клетки шахматной доски */
...
int main(void)
{
	...
	printf("%4s %10s %10s ", "cell", "current", "total");
	printf("%15s", "percentage of\n");
	printf("%4s %10s %10s ", "", "corn", "corn");
	printf("%15s", "world crops\n");

	total = current = 1.0;		/* отсчет идет с 1 зернышка */
	printf("%4d %10.2e %10.2e %10.2e\n", count, current, total, total / CROP);
	while (count < SQUARES)
	{
		printf("%4d %10.2e %10.2e %10.2e\n", count, current, total, total / CROP);
		count = count + 1;
...
/*
$ ./wheat 
cell    current      total  percentage of
           corn       corn    world crops
